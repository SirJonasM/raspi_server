...
pqclean = ctypes.CDLL('./build/libpqc_kyber1024.so')

# Constants
KYBER_PUBLICKEYBYTES = 3168 
KYBER_SECRETKEYBYTES = 1568 
KYBER_CIPHERTEXTBYTES = 1568 
KYBER_SHAREDSECRETBYTES = 32

# Flask Blueprint
kyber1024_rust_blueprint = Blueprint("kyber1024rust", __name__)
...
@kyber1024_rust_blueprint.route("/dilithium2", methods=["POST"])
...
@kyber1024_rust_blueprint.route("/dilithium3", methods=["POST"])
def kyber_dilithium():
    try:
        data = request.json
        if not data or 'client_public_key' not in data:
...
@kyber1024_rust_blueprint.route("/dilithium5", methods=["POST"])
...
@kyber1024_rust_blueprint.route("/falcon512", methods=["POST"])
def falcon_512():
    try:
...
@kyber1024_rust_blueprint.route("/falcon1024", methods=["POST"])
def falcon_1024():
    try:
        data = request.json
        if not data or 'client_public_key' not in data:
...
@kyber1024_rust_blueprint.route("/rainbowI", methods=["POST"])
...
@kyber1024_rust_blueprint.route("/rainbowIII", methods=["POST"])
...
@kyber1024_rust_blueprint.route("/rainbowV", methods=["POST"])
def kyber_rainbowV():
    try:
        data = request.json
        if not data or 'client_public_key' not in data:
