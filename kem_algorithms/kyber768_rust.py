...
pqclean = ctypes.CDLL('./build/libpqc_kyber768.so')

# Constants
KYBER_PUBLICKEYBYTES = 2400 
KYBER_SECRETKEYBYTES = 1184 
KYBER_CIPHERTEXTBYTES = 1088 
KYBER_SHAREDSECRETBYTES = 32

# Flask Blueprin-
kyber768_rust_blueprint = Blueprint("kyber768rust", __name__)
...
@kyber768_rust_blueprint.route("/dilithium2", methods=["POST"])
...
@kyber768_rust_blueprint.route("/dilithium3", methods=["POST"])
def kyber_dilithium():
    try:
        data = request.json
        if not data or 'client_public_key' not in data:
...
@kyber768_rust_blueprint.route("/dilithium5", methods=["POST"])
...
@kyber768_rust_blueprint.route("/falcon512", methods=["POST"])
def falcon_512():
    try:
...
@kyber768_rust_blueprint.route("/falcon1024", methods=["POST"])
def falcon_1024():
    try:
        data = request.json
        if not data or 'client_public_key' not in data:
...
@kyber768_rust_blueprint.route("/rainbowI", methods=["POST"])
...
@kyber768_rust_blueprint.route("/rainbowIII", methods=["POST"])
...
@kyber768_rust_blueprint.route("/rainbowV", methods=["POST"])
def kyber_rainbowV():
    try:
        data = request.json
        if not data or 'client_public_key' not in data:
